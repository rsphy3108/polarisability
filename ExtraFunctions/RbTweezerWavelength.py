...
import sys
sys.path.append('..')
from AtomFieldInt_V3_new import (dipole, atom, c, eps0, h, hbar, a0, e, me, 
    kB, amu, Eh, au)
...
def get_scattering_rates(I, wls):
    ...
    deltaRbD2 = 2*np.pi*c * (1/wls - 1/Rb.rwS[5]) # detuning from D2 (rad/s)
    IsatRbD1 = 4.484 *1e-3 *1e4 # saturation intensity for D1 transition, pi polarised
    IsatRbD2 = 2.503 *1e-3 *1e4 # saturation intensity for D2 transition, pi polarised
    RbRsc = 0
    for vals in [[Rb.lwS[0], deltaRbD1, IsatRbD1], [Rb.lwS[5], deltaRbD2, IsatRbD2]]:
        RbRsc += vals[0]/2. * I/vals[2] / (1 + 4*(vals[1]/vals[0])**2 + I/vals[2])
    ...
    return CsRsc, Cst, RbRsc, Rbt, trapdepths
...
CsRsc, Cst, RbRsc, Rbt, Us = get_scattering_rates(Rbintsts, np.array(list(wavels) + [880.2e-9]))
# print("%.1f \t %.3g \t %.3g \t %.3g \t %.3g \t %.3g \t %.3g"%(wavels[i]*1e9, Us[0][i]/kB*1e3, CsRsc[i], 1e3/CsRsc[i], Rbintsts[i]*np.pi*1.2e-6**2*0.5e3, RbRsc[i], 1e3/RbRsc[i]))
...
CsRsc_Cs, Cst_Cs, RbRsc_Cs, Rbt_Cs, Us_Cs = get_scattering_rates(Csintsts, 1064e-9)
...
ax3.set_ylim(1, 1e4)
ax3.set_xlim(wavels[0]*1e9, wavels[-2]*1e9)
